...
export default {
  ...
  runFormat(text) {
    let command = "mix";
    let args = ["format", "-"];
    let options = this.getCommandOptions(text);
  ...
  getCommandOptions(text) {
    let options = { input: text };

    // set cwd to editor root path so ".formatter.exs" file can be used
    if ((path = main.getActiveTextEditorRootPath())) {
      options.cwd = path;
    }
